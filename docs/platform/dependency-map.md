...
casehub-parent              (BOM — publish first; all others import it)
  ...
  casehub-langchain4j       (depends on casehub-platform-api, casehub-ledger-api, casehub-neocortex-rag-api, casehub-engine-api — enterprise enrichment for LangChain4j)
...
| `casehub-platform-api` | `casehub-langchain4j` | `audit-core`, `tenancy-core` | `CurrentPrincipal` for tenant isolation |
| `casehub-ledger-api` | `casehub-langchain4j` | `audit-core`, `governance-core` | `CaseLedgerEntry` for tamper-evident audit |
| `casehub-neocortex-rag-api` | `casehub-langchain4j` | `hybrid-search-core` | `CaseContextRetriever` for hybrid SPLADE+dense retrieval |
| `casehub-engine-api` | `casehub-langchain4j` | `governance-core` | OversightGateService, trust routing (optional) |
| `casehub-platform-agent-api` | `casehub-langchain4j` | `agents-core` | `AgentProvider` SPI for ChatModel ↔ AgentProvider bridge |
| `langchain4j-core` | `casehub-langchain4j` | all `-core` modules | LC4j SPI interfaces (ChatModelListener, ChatMemoryStore, ContentRetriever, EmbeddingStore) |
| `langchain4j-agentic` | `casehub-langchain4j` | `audit-core`, `governance-core` | `AgentListener` for agent invocation monitoring |
