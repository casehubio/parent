...
- [ ] `ARC42STORIES.MD` — create in project root. Do NOT leave as a bare stub with
  `🔲 pending (TBD)` on every chapter. Populate at minimum:
  - §1 Introduction (description, stakeholders, quality goals)
  - §4 Solution Strategy (layer taxonomy, architectural patterns)
  - §9.1 Journey overview table (all journeys with descriptions, chapters, status)
  - §9.2 Chapter index (mermaid flowchart showing dependencies)
  - §9.3 Per-chapter entries — at least "What this delivers" and "Issues" for each
  Copy the format from any completed repo's ARC42STORIES.MD (e.g. `casehub-platform`).
  Later chapters can have lighter entries that get fleshed out as work approaches them.
...
## 7. Parent Platform Docs

- [ ] **Repository Map** — add row to `docs/platform/overview.md`: `| casehub-<name> | [casehubio/<name>](...) | one-liner | Tier |`
- [ ] **Build / Dependency Order** — add the new repo in the correct topological position in `docs/platform/overview.md`.
...
- [ ] **`build/modules-core.csv`** (foundation/orchestration/integration tier) or
  ...
  **Special cases** (handled by the generator — no extra action needed):
  ...
  - `fsitrading`, `soc` → cloned from `mdproctor/` org using `GH_PAT`

- [ ] **`dashboard.yml`** — add `casehubio/<name>` to the `REPOS` printf list.

- [ ] **`pr-dashboard.yml`** — add `casehubio/<name>` to the `REPOS` printf list.

- [ ] **All modules must be in CI.** Every repo must be in `modules-core.csv` or
  `modules-applications.csv`. There is no local-only option — modules outside CI are
  invisible to the ecosystem and silently drift out of sync with upstream changes.
...
## 12.5. casehub-all Aggregator

- [ ] If a `casehub-all` aggregator repo exists (at `~/claude/casehub/all/`), add the new
  repo to the aggregator's pom.xml or module list. This ensures full-stack builds include
  the new repo. Without it, integration issues surface late.

---

## 13. Per-Repo Guide (`docs/guide.md`)

- [ ] Create `docs/guide.md` in the new repo with two headed sections:
  - **Consumer Guide** — what this module does, modules to depend on, SPIs to implement,
    key types, configuration, lifecycle states, boundary rules (what NOT to do)
  - **Contributor Guide** — module architecture table, internal CDI wiring, test infrastructure,
    migration conventions, dependencies (depends on / depended on by), current state
- [ ] Follow the pattern of the closest existing guide (e.g., `casehub-work/docs/guide.md`
  for foundation tier, `casehub-clinical/docs/guide.md` for application tier).
  Guides must not be empty stubs — fill all sections with real content:
  - **Consumer Guide:** getting started, module reference, configuration, graceful degradation,
    code examples. A new developer should be able to use the repo from this document alone.
  - **Contributor Guide:** module architecture table (all modules with artifact, package,
    responsibility), CDI conventions, Spring conventions (if applicable), test conventions,
    dependencies (depends on / depended on by), build commands, expansion policy (if applicable).
- [ ] Add the repo to `parent/scripts/guide-sync-config.json`
- [ ] Run `python3 scripts/sync-guides.py --repo <name>` to verify sync to `docs/repos/`
- [ ] Add `guide-updated` dispatch to the repo's `publish.yml` for CI-triggered sync:
  ```yaml
  - name: Sync guide to parent
    run: |
      gh api repos/casehubio/parent/dispatches \
        -f event_type=guide-updated \
        -f client_payload[repo]=${{ github.repository }}
  ```

---

## 13.5. HANDOFF.md (optional)

- [ ] If the repo will be handed off to another developer for implementation, create
  `HANDOFF.md` documenting:
  - What's done (scaffold, docs, CI, specs)
  - What's NOT done (implementation, outstanding integration steps)
  - How to work through the implementation (reference ARC42STORIES chapters)
  - Key principles to maintain (architectural constraints, positioning)
  - Contacts (architecture owner, external stakeholders)
  - Open questions and decisions still pending

---

## 13.6. VISION.md / STRATEGY.md (optional)

- [ ] For externally-facing repos or repos positioned for community engagement, create:
  - `docs/VISION.md` — brief explaining what the repo does and why, written for external
    consumption. Problem-driven: concrete scenarios, not feature lists.
  - `STRATEGY.md` — positioning, expansion policy, competitive analysis. Internal-facing
    but defensible if shared externally.
...
- [ ] Stage all parent changes: pom.xml, docs/platform/overview.md, APPLICATIONS.md, docs/repos/<name>.md,
...
| ARC42STORIES.MD is just a stub | Sessions have no roadmap; chapters and journeys are not populated — handover and work-start skills have nothing to route from | Populate at least journey overview, chapter index, and per-chapter "What this delivers" entries — even brief ones for later phases |
| Contributor/consumer guides are empty | New contributors have no orientation; consumers don't know which modules to depend on | Use the closest existing guide as a template; fill all sections with real content |
| Repo not in casehub-all aggregator | Full-stack builds skip the repo; integration issues surface late | Add to casehub-all aggregator pom.xml or module list after parent registration |
| Stub classes don't compile | `mvn validate` passes but `mvn compile` fails on unresolvable types in stubs | Verify `mvn compile` — not just `validate` — before marking scaffold as complete. Check stub classes reference APIs that actually exist in the pinned dependency versions. |
| VISION.md or STRATEGY.md not written for externally-facing repos | No shareable document for community engagement; positioning is ad-hoc | Write a problem-driven brief that explains what the repo does and why, suitable for external consumption. Run an adversarial review on it. |
