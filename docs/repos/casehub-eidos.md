...
- eidos#46 — Eval baseline and behavioral validation — in progress. Phase 1 (structural baseline) and Phase 3 (pair-contrast behavioral judge via `AgentProviderChatModel` bridge to Claude CLI) implemented. `DispositionAxis.jsonKey()` and `description()` added to api. All compute-path migrations from `String` to `DispositionAxis` complete.
