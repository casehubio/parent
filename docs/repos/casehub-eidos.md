...
|---|---|---|---|
| `api/` | `casehub-eidos-api` | Pure Java, no CDI | SPIs and domain types — `AgentDescriptor`, `AgentRegistry`, `CapabilityHealth`, `VocabularyRegistry`, `SystemPromptRenderer`, `AgentStateStore`; top-level types: `AgentPromptContext`, `DegradationReason`, `GoalContext`, `Resource`, `AgentMatch` (result of `find()`: descriptor + nullable `ResolvedCapability`), `ResolvedCapability` (capability + `MatchDegree`), `MatchDegree` (sealed interface: Exact, Plugin(depth), Specialization(depth), None; implements `Comparable<MatchDegree>` with OWLS-MX ordering); vocabulary types: `VocabularyTerm` (interface — replaced record), `VocabularyRegistrar` (@FunctionalInterface CDI SPI), `VocabularyMetadata` (annotation on enum classes), `DispositionAxis` (enum: SOCIAL_ORIENTATION, RULE_FOLLOWING, RISK_APPETITE, AUTONOMY, CONFLICT_MODE — 5th axis added in eidos#38; `jsonKey()` → camelCase JSON key for LLM responses; `description()` → human-readable axis description for judge prompts) |
...
Declares a named capability with an optional `qualityHint` (Double, 0–1), `description` (String, ≤500 chars, eidos#77), `epistemicDomains` map (domain → confidence, e.g. `{"java": 0.95, "rust": 0.42}`), and `excludedDomains` (Set<String>, eidos#62 — domains the agent refuses to handle). The `epistemicDomains` map qualifies *how well* the agent handles the declared capability in specific subject domains — it is not a list of separate capabilities. `excludedDomains` is validated against `epistemicDomains` at construction (no overlap allowed).
...
**`excludedDomains` (eidos#62):** `Set<String>` field — declares task domains the agent refuses to handle. Validated at construction: no overlap with `epistemicDomains` keys allowed (throws `AgentValidationException`). Persisted in JPA as `@ElementCollection`. Used by `AgentQuery.taskDomain` pre-filter: `CapabilityHealth` checks declared exclusions and returns `Excluded(DECLARED)` status. `AgentDescriptorComparator` compares this field for drift detection.

**Validation (compact constructor):** `name` required ≤100; `description` optional ≤500; `costHint` optional ≤200; list items in `inputTypes`/`outputTypes`/`tags`/`excludedDomains` ≤200 each; `epistemicDomains` keys ≤200. Same character-set rules as `AgentDescriptor`. Throws `AgentValidationException` on violation.
...
Utility in `casehub-eidos-api` for drift detection between desired and actual descriptors. `compare(AgentDescriptor desired, AgentDescriptor actual)` returns `ComparisonResult(List<FieldDrift> drifts)` — empty list means match. `FieldDrift(String field, String desiredValue, String actualValue)`. Compares 16 top-level fields (13 simple + axisVocabularies + disposition + capabilities), 6 disposition fields, and 10 per-capability fields (including `excludedDomains`). Used by consumers to detect when a registered descriptor has drifted from the desired persona configuration.

### AgentDescriptorRegistrar SPI

`@FunctionalInterface` SPI in `api/spi/`: `List<AgentDescriptor> descriptors()`. CDI beans implementing this interface are discovered at startup and their descriptors bulk-registered. Two bootstrap implementations in runtime: `AgentDescriptorBootstrap` (blocking, `@Observes StartupEvent`, gated on reactive disabled) and `ReactiveAgentDescriptorBootstrap` (reactive, gated on `casehub.eidos.reactive.enabled=true`). Both delegate to `DescriptorCollector.collectAndValidate()` which validates no duplicate `(agentId, tenancyId)` pairs.

**`ClasspathYamlDescriptorRegistrar`** — `@ApplicationScoped` implementation scanning `META-INF/eidos/descriptors.yaml` from the classpath. Loads `DescriptorFile` YAML with full `AgentDescriptor` fields including `briefing`, `axisVocabularies`, `disposition`, and `capabilities` (with `excludedDomains`). Multiple YAML files across JARs are merged.

### AgentRegistry / ReactiveAgentRegistry

SPIs for register, findById, and find(AgentQuery). `find()` returns `List<AgentMatch>` — each entry carries the matched `AgentDescriptor` + nullable `ResolvedCapability` (the declared capability that matched and the OWLS-MX `MatchDegree`). `ResolvedCapability` is non-null when `AgentQuery.capabilityName()` was specified; null for slot-only or `all` queries. Results are ordered by match quality (Exact < Plugin < Specialization) when capability is queried. `CapabilityResolver.resolve()` provides the shared subsumption resolution returning `ResolvedCapability(capability, degree)`. `AgentQuery` carries: `slot`, `capabilityName`, `tenancyId` (required — all queries are tenancy-scoped), and `taskDomain` (optional pre-filter by `excludedDomains`). Factory: `byCapabilityAndDomain(capabilityName, taskDomain, tenancyId)` — agents whose declared `excludedDomains` contain the `taskDomain` are filtered out before capability matching. `JpaAgentRegistry` is the `@ApplicationScoped` default; `JpaReactiveAgentRegistry` is build-gated via `casehub.eidos.reactive.enabled`. `InMemoryAgentRegistry` activates from `casehub-eidos-memory`.

### CapabilityHealth / ReactiveCapabilityHealth

SPI: `probe(AgentDescriptor, capabilityTag, ProbeContext)` → `CapabilityStatus`. Sealed interface with statuses: `Ready`, `Degraded`, `Unavailable`, `EpistemicallyWeak`, `Excluded(domain, ExclusionSource, declineCount)` (ExclusionSource: DECLARED or LEARNED), `BehavioralViolation(violations, ViolationKind)` (ViolationKind: PER_DIMENSION or AGGREGATE). `DegradationReason` is a top-level type in `casehub-eidos-api` — not nested inside `CapabilityHealth`.
...
JPA/Flyway — version range V1–V999 in `classpath:db/eidos/migration`. Current migrations: V1 (initial schema), V2 (agent degradation state), V4 (capability specialization table), V5 (behavioral signal table with `signal_type` discriminator), V6 (capability description column).

---

## Current State

- Phases 1 and 2 complete and merged to main (8 modules, all green).
...
- eidos#23 — Real-world agent profile library — complete. 10 YAML profiles (Belbin/DISC grounded; 8 original + 2 data-engineer profiles with capability descriptions), `AgentProfileLoader` with Stage 0 pair isolation validation, `ProximityJudge` (semantic proximity scoring), three-stage personality preservation measurement (`VocabularyExpressivenessJudge` / `TraitExpressionJudge` / `PairContrastJudge`).
...
- eidos#62 — `excludedDomains` on `AgentCapability` — complete. `Set<String>` field declaring task domains the agent refuses. Validated against `epistemicDomains` (no overlap). JPA-persisted as `@ElementCollection`. V4 Flyway migration: `capability_specialization` table. `CapabilityHealth` returns `Excluded(DECLARED)` for matching domains.
- eidos#77 — `description` field on `AgentCapability` — complete. Up to 500 chars. Multi-format rendering: MARKDOWN/PROSE surface capability names and I/O types; `A2A_CARD` carries full routing signals including description. Enrichment fallback when LLM unavailable.
- eidos#95 — Data-engineer eval profiles — complete. 2 new profiles (`data-engineer-autonomous`, `data-engineer-directed`) with AUTONOMY variant pair. 3 capabilities with full descriptions: `pipeline-orchestration`, `data-quality-assessment`, `schema-evolution`. Brings total to 10 profiles and 3 variant pairs.
- `AgentDescriptorComparator` — drift detection utility. Compares 16 top-level fields, 6 disposition fields, 10 per-capability fields. Returns `ComparisonResult(List<FieldDrift>)` with `matches()` convenience method.
- `AgentDescriptorRegistrar` SPI + `ClasspathYamlDescriptorRegistrar` — YAML persona loading from `META-INF/eidos/descriptors.yaml`. Bootstrap (blocking + reactive-mode) registers all discovered descriptors at startup with duplicate `(agentId, tenancyId)` validation.
- `CapabilityResolver` utility — shared subsumption resolution for probe and recording paths. `match()` returns `MatchDegree` (Exact/Plugin/Specialization/None); `resolve()` returns best `ResolvedCapability` from a list. Eliminates duplicated matching logic between `CapabilityHealth.probe()` and `BehavioralSignalStore` lookups.
