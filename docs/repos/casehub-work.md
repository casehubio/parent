...
| `ExclusionPolicy` | SPI in `casehub-work-api`: `isExcluded(String userId, String excludedUsers)`. `CommaSeparatedExclusionPolicy` is the `@DefaultBean`. Override with `@Alternative @Priority(1)` for custom exclusion logic (LDAP, role-based, time-window — see #185). `excludedUsers` field on template + WorkItem; snapshotted at instantiation. |
