...
| 10 | IND deadline enforcement | No absolute FDA deadline enforcement on regulatory submission WorkItems; expiry relied on SLA only | ✅ complete (clinical#83, 2026-06-22) — `regulatory-submission.yaml` `humanTask` binding uses `expiresAtExpression` (engine#549 SPI: `WorkItem.expiresAt = ae.reportedAt + window`, exact absolute FDA deadline). `ClinicalIndReportingBreachPolicy @ApplicationScoped` (stateless two-tier `SlaBreachPolicy`: EscalateTo regulatory-leadership 48h). `RegulatorySubmissionCompletedListener` sets `FILED` on `GoalReached`. `RegulatorySubmissionBreachListener` sets `DEADLINE_MISSED` on `WorkItemLifecycleEvent.ESCALATED` via `CallerRef.parse()`. `IndReportFiledLedgerEntry` + `IndReportBreachLedgerEntry` (V2026/V2027, qhorus datasource). `RegulatorySubmissionStatus.DEADLINE_MISSED` enum value. `ClinicalComplianceSupplement` adds `regulatorySubmissionFiled()` and `regulatorySubmissionBreach()` factory methods. `RegulatorySubmissionLedgerWriter` gains `writeFiledEntry()` and `writeBreachEntry()`. Requires `ExpressionEngine.extractString()` SPI from engine#549. |
