...
- **`agent-claude-langchain4j/` (shipped — platform#100):** LangChain4j `ChatModel`/`StreamingChatModel` bridge backed by `AgentSession`. Two adapters: `ClaudeAgentChatModel` (`@Alternative @Priority(10) @ApplicationScoped` — stateless, fresh session per call, system prompt caching via Claude CLI) and `AgentSessionChatModel` (plain wrapper — caller supplies session, multi-turn, caller owns lifecycle). **Incompatible with `engine.Agent`** — `engine.Agent.buildResponseFormat()` always forces `ResponseFormatType.JSON`; `ClaudeAgentChatModel` throws `UnsupportedFeatureException` for JSON format. Use a JSON-capable `ChatModel` with `engine.Agent`. `casehub.platform.agent.langchain4j.closeTimeout` (default PT30S). No quarkus:build goal — library module.
- **`agent-langchain4j/` (future option):** provider-agnostic non-Claude LLMs (OpenAI, Gemini, etc.). Requires `AgentEvent` extension (`ToolCall`/`ToolResult` variants) — current SPI assumes opaque execution model where tool calls are handled inside the subprocess. The Claude-specific LangChain4j path is covered by `agent-claude-langchain4j/`.

For Claude-native work, keep `agent-claude/` or `agent-claude-langchain4j/` depending on whether you need the `ChatModel` interface. For non-Claude LLMs, `agent-langchain4j/` is the future path — but extending `AgentEvent` first is the prerequisite.
...
| `agent-claude/` | ✅ shipped (#55, #58) | `ClaudeAgentProvider @ApplicationScoped` + `ClaudeAgentClient @Startup` — activates by classpath presence; requires Claude CLI; concurrent-session semaphore (configurable); wall-clock timeout; three exception types: `AgentProcessException`, `AgentSessionLimitException`, `AgentTimeoutException`. **Multi-turn (#58):** `ClaudeAgentSession` — IDLE/ACTIVE/CLOSED state machine; per-turn wall-clock timeout with `AgentTimeoutException`; true-drain `close(Duration)` via `CompletableFuture`; `interrupt()` fire-and-forget (TOCTOU-guarded); semaphore held for session lifetime (19 unit tests). |
| `agent-claude-langchain4j/` | ✅ shipped (#100) | LangChain4j `ChatModel`/`StreamingChatModel` bridge backed by `AgentSession`. `ClaudeAgentChatModel` (`@Alternative @Priority(10) @ApplicationScoped` — stateless, fresh session per call) + `AgentSessionChatModel` (multi-turn, caller owns lifecycle). **Incompatible with `engine.Agent`** which forces `ResponseFormatType.JSON`. `casehub.platform.agent.langchain4j.closeTimeout` (default PT30S). No quarkus:build goal. |
